...
- **Auditable** — every result includes a **§-cited computation trace** (which section, what amount).
- **Statutorily correct rounding** — rounds per §288A/§288B (to the nearest ₹10), like the real return.
...
- `GET /health` · `GET /ready` · `GET /version` — liveness, readiness, and supported years.
...
```json
{
  ...
  "result": {
    "recommended": "new",
    "old": { "taxPayable": "…", "trace": [/* §-cited steps */] },
    "new": { "taxPayable": "…", "trace": [/* §-cited steps */] }
  }
}
```
