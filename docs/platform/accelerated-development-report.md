...
| Web tests             | **130 files / 2490 tests passed**, 0 failures        |
...
| Android debug APK     | **BUILD SUCCESSFUL in 2m 57s**, 56.2 MB              |

The production build emits all six development proxy routes as dynamic
handlers. That is expected: they exist in the bundle and refuse at runtime.
The refusal was verified empirically in VYORA-PLATFORM-014 — with all four
flags forced `true` and `NODE_ENV=production`, every one returns `404`.

```
ƒ /api/vyora-dev/parties
ƒ /api/vyora-dev/parties/[partyId]
ƒ /api/vyora-dev/parties/[partyId]/credits
ƒ /api/vyora-dev/parties/[partyId]/payments
ƒ /api/vyora-dev/parties/[partyId]/statement
ƒ /api/vyora-dev/parties/[partyId]/summary
```

The EMI reconciliation test — historically a flake on this two-core machine
under concurrent database load — passed with PostgreSQL, the API and a dev
server all running. It is no longer reported as a known-flaky item on the
strength of this run alone, but it did not fail here.
...
```
vyora-mobile/android/app/build/outputs/apk/debug/app-debug.apk   56.2 MB
BUILD SUCCESSFUL in 2m 57s

package        in.esytol.vyora   versionCode 1   versionName 0.1.0
minSdk 24      targetSdk 36      compileSdk 36
ABI            lib/arm64-v8a/
native libs    libexpo-sqlite.so, libhermesvm.so, libreactnative.so
permissions    INTERNET, ACCESS_NETWORK_STATE
cleartext      usesCleartextTraffic=true — from the DEBUG manifest only
```

`libexpo-sqlite.so` and `libhermesvm.so` being present is the evidence that the
ledger store and the JS engine linked. There is no `index.android.bundle`
inside, which is correct for a debug build — Metro serves the JS.

Two earlier attempts failed with `IOException: The filename, directory name, or
volume label syntax is incorrect`. The cause was **not** the space in the
repository path, which was the obvious suspect: `android/local.properties` had
been written with single backslashes, and a Java properties file reads those as
escape sequences, so `sdk.dir` was malformed. The stack trace named
`SdkLocator.validateSdkPath`. Forward slashes fix it. Worth remembering — the
error text points at the filesystem and says nothing about escaping.
...
API server, Next dev server and all Gradle/native build processes stopped.
`.next` removed — it held a build made with flags forced on. The synthetic
database was re-seeded, discarding the end-to-end rows, and PostgreSQL stopped.
No ports left listening; no node, java or postgres processes remain.

`android/` is generated by `expo prebuild` and is gitignored, as Expo's
continuous native generation intends — `app.json` is the configuration of
record. The APK is a local build artefact and is not committed.
