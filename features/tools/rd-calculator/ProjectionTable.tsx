...
import { projectionToCSV, downloadCSV, formatINR } from "@/lib/rd";

interface ProjectionTableProps {
  rows: RDProjectionRow[];
  monthlyDeposit: number;
}

const INITIAL_VISIBLE = 12;

const COLUMNS = ["Month", "Total Deposited", "Interest Earned", "Balance"] as const;

export function ProjectionTable({ rows, monthlyDeposit }: ProjectionTableProps) {
  ...
  const handleDownload = useCallback(() => {
    const csv = projectionToCSV(rows, monthlyDeposit);
    downloadCSV(csv, "rd-projection.csv");
  }, [rows, monthlyDeposit]);

  return (
    <section aria-labelledby="rd-projection-heading">
      <div className="mb-4 flex items-center justify-between gap-4">
        <h2 id="rd-projection-heading" className="text-lg font-semibold text-gray-900">
          Month-wise Growth
        </h2>
        <button
          type="button"
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200">
        <table className="min-w-full text-sm" aria-label="Month-wise RD growth schedule">
          <thead>
