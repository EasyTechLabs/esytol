/**
 * Content Agent — reads the registry, the Learn Center and traffic together.
 *
 * This is the only agent that sees the *product* as well as the analytics, so it
 * finds structural gaps no single provider can: tools with no supporting content,
 * articles that link to nothing, missing FAQs (no FAQ schema), and clusters that
 * are large enough to deserve a Collection.
 */
...
const COLLECTION_MIN_TOOLS = 3;

export const contentAgent: Agent = {
  key: "content",
  label: "Content Agent",
  status: "active",
  watches: "Learn Center + registry + tool usage — content gaps, FAQs, internal links",
  run(ctx: AgentContext): Recommendation[] {
    const out: Recommendation[] = [];
    const { tools, articles } = ctx;

    // Which tools already have supporting content?
    const covered = new Set<string>();
    for (const a of articles) for (const slug of a.relatedToolSlugs) covered.add(slug);

    // Traffic per tool page, for prioritisation.
    ...
    const noFaq = articles.filter((a) => a.faqs.length === 0).slice(0, PER_RULE_LIMIT);
    for (const a of noFaq) {
      out.push(
        buildRecommendation({
    ...
    const noLinks = articles
      .filter((a) => a.relatedToolSlugs.length === 0)
      .slice(0, PER_RULE_LIMIT);
    ...
    for (const [category, count] of [...byCategory.entries()].sort((a, b) => b[1] - a[1])) {
      if (count < COLLECTION_MIN_TOOLS) continue;
      out.push(
        buildRecommendation({
    ...
    const hotArticles = ctx.growth.analytics.data.topPages
      .filter((p) => p.page.startsWith("/learn/"))
      .sort((a, b) => b.views - a.views)
      .slice(0, 3);

    for (const p of hotArticles) {
